...
## UI/UX 設計與文字規範 (UI/UX Design Guidance)

* **極致簡潔、直覺好用**：介面應保持清爽，避免不必要的文字與視覺元素。
* **圖示優先**：若 SVG / Icon 已能清楚表達功能，**優先使用圖示而非文字**。例如 ✓、✕、＋、⌄、⋮ 等，能不用文字就不要加文字。
* **文字精煉**：無法單靠圖示清楚表達時，按鈕、選項、標籤與導覽文字優先使用單詞或極短片語。
* **避免文字堆砌**：嚴禁在按鈕、選項或標籤中塞入長篇說明、操作指引或不必要的背景資訊。
